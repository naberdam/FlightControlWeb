...
namespace FlightControl
{
    public class SqliteDataBase : IDataBase
    {
        ...
        public string AddFlightPlan(FlightPlan flightPlan)
        {
            ...
            if (!CheckObjects.CheckFlightPlan(flightPlan)) { return null; }
            OpenConnection();
            // Random id.
            string id = SetRandId();
        ...
        public async Task<FlightPlan> GetFlightPlanById(string id)
        {
            ...
            for (; i < listOfExternalServers.Count; ++i)
            {
                ...
                if (!CheckObjects.CheckFlightPlan(flightPlanFromExternalServer)) { continue; }
        ...
        {
            OpenConnection();
            SqliteCommand addFlightsFromExternalServersSQLTableCommand = SetSqliteCommand(
                "INSERT INTO FlightsFromExternalServersSQL VALUES (@ServerId , @IdFlight)");
            addFlightsFromExternalServersSQLTableCommand.Parameters.AddWithValue(
                "@ServerId", idOfServer);
            addFlightsFromExternalServersSQLTableCommand.Parameters.AddWithValue(
                "@IdFlight", flightId);
            try
            {
                addFlightsFromExternalServersSQLTableCommand.ExecuteReader();
            }
            catch (Exception) { }
            CloseConnection();
        }
